...
const AdminLogin = () => {
  ...
  const handleClick = async (e) => {
    e.preventDefault();
    try {
      await axios.post("http://localhost:8080/api/v1/taskManager/adminLogin", {
        email,
        password,
      });
      ...
      <div className="h-[800px] flex items-center justify-center">
        <div className="p-20 bg-gray-300 rounded-lg shadow-md border-4 border-black">
          ...
          <form onSubmit={handleClick} className="mt-5 flex flex-col">
            ...
            <button
              type="submit"
              className="m-8 bg-blue-400 rounded-xl py-2 hover:bg-blue-600 text-white border-2 border-black "
            >
              Sign In
            </button>
          </form>
